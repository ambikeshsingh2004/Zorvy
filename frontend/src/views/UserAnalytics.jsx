...
export default function UserAnalytics({ user: currentUser }) {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      const res = await axios.get(`/api/user-analytics/analyze/${userId}?${params.toString()}`, {
    ...
    <Layout user={currentUser} setUser={() => {}}>
      <div className="max-w-[1440px] mx-auto flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <button onClick={() => navigate('/user-analytics')} className="text-zinc-500 hover:text-white text-xs font-medium transition-colors mb-2 inline-flex items-center gap-1">
              ← Back to Users
            </button>
            <h2 className="text-3xl font-semibold text-white tracking-tight" style={{ fontFamily: 'Syne, sans-serif' }}>
              {data?.user?.name || 'User'} — Analytics
            </h2>
            <p className="text-zinc-500 text-sm mt-1 font-medium">{data?.user?.email} · {data?.user?.role}</p>
          </div>
        </div>
                ...
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="border-b border-white/5">
                      ...
                      <th className="pb-3 text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Notes</th>
                    ...
                    {data.records.length === 0 ? (
                      ...
                      data.records.map(r => (
                        <tr key={r.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                          ...
                          <td className="py-3 text-zinc-500 text-xs max-w-[200px] truncate">{r.notes || '—'}</td>
                          <td className="py-3">
                            {r.deleted_at ? (
        ...
        {loading && (
          <div className="text-center py-12 text-zinc-500 text-sm">Analyzing user data...</div>
        )}
      </div>
    </Layout>
  );
